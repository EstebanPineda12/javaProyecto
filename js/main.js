...
function monedaCabio3() {
    ...
    alert("Este algortimo convierte el peso colombiano en dolar, euro o bolivar venezolano");

    let pesoColombiano = parseFloat(prompt("Ingrese la cantidad de pesos colombianos que desea convertir"));
    let tasa = parseInt(prompt("Seleccione cque monedad desea convertir: USD = 1, EUR = 2, BOL = 3"));
    let conversion;

    switch (tasa) {
        case 1:
            conversion = pesoColombiano * tasaCambio.USD
            alert(pesoColombiano + " pesos colombianos son " + conversion + " Dolares");
            break;
        case 2: 
        conversion = pesoColombiano * tasaCambio.EUR
        alert(pesoColombiano + " pesso colombianos son " + conversion + " Euros");
            break;

        case 3: 
        conversion = pesoColombiano * tasaCambio.BOL
        alert(pesoColombiano + " pesos colombianos son " + conversion + " Bolivares");
            break;
        default:
            alert("dato ingesado no valido")
            break;
    }
}
...
function parImpar() {
    ...
    let numero = parseInt(prompt("Ingrese el numero el que desea saber si es par o impar"));
    if (numero % 2 === 0) {
...
function menorTresn() {
    ...
    if (X== Y && X==Z && Y==Z) {
        ...
        alert(Y + " es el numero menor");
    } else if (Z<X && Z<Y){
        alert(Z + " es el numero menor");
    } else if (X<Y && X<Z){
        alert(X + " es el numero menor");
    }
}

function aprobarOno() {
    alert("Quieres saber si aporbaste o reprobaste, descubrelo");

    let nombre = prompt("Ingresa tu nombre");
    let materia= prompt("Introduce la materia");
    let calificaciones =[];
    ...
    if(promedio >= 6.1){
        alert("Querido estudiante " + nombre + " en la materia de " + materia + " aprobo con un promedio de " + promedio + " felicitaciones");
    }else {
        alert("Querido estudiante " + nombre + " en la materia de " + materia + " reprobo con un promedio de " + promedio + " lo lamento");
    }
}
    
function capitalBank(){
    alert("Invierte ahora en banco sena")

    let dineroIng = parseInt(prompt("Ingrese el valor que desea invertir"));
    let anios = parseInt(prompt("Ingrese cuantos años quiere invertir su dinero"));
    let ganacias;
    let x;
    let y;
    ...
    x = anios * 12;
    y= x * 0.007;
    ganacias = dineroIng * y;

    alert("Vas a recibir " + ganacias + " ganacias despues de " + anios + " años");
